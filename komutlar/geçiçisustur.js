...
module.exports.run = async (bot, message, args) => {
  ...
  //nt!geçicisustur@üye 1s/m/h/d | 1s = 1 saniye , 1m = 1 dakika , 1h = 1 saat, 1d = 1 gün

  let tomute = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
  if(!tomute) return message.reply("Bu Komutu Kullanmak İçin : nt!geçici-sustur <Kullanıcı> <Süre> Olarak Yazmalısınız.");
...
let muterole = message.guild.roles.find(r => r.name === "Muted");
  ...
  //end of create role
  let mutetime = args[1];
  if(!mutetime) return message.reply("Bu Komutu Kullanmak İçin : nt!geçici-sustur <Kullanıcı> <Süre> Olarak Yazmalısınız.");
